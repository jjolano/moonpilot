...
class MoonpilotCurvature:
  """Stateless response-aligned curvature correction called once per control frame."""

  def update(self, model, desired_curvature: float, *, v_ego: float, lat_delay: float, model_recv_time: float, now: float, model_valid: bool) -> float:
    """Return the decoded request plus a bounded path correction, or the request unchanged."""
    if not model_valid:
