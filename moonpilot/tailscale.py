...
and `moonpilot/tailscaled.py` supervises it. Binaries, the control socket, tailscaled's var root
and its node identity, login session and prefs stay under `<data_root>/tailscale`; a factory reset
may log the device out. On PC, a legacy state file can migrate to `<persist_root>/tailscaled.state`
instead. This module is the shared vocabulary for both, plus the encode/decode of the one string
param the supervisor and the settings panels talk through.
...
def legacy_state_path() -> str:
  """The data-root state file, canonical on device and the migration source on PC."""
  return os.path.join(root(), "tailscaled.state")


def persistent_state_path() -> str:
  """The PC-only state path that survives a factory reset."""
  return os.path.join(paths.persist_root(), "tailscaled.state")


def _persist_writable() -> bool:
  """Whether the existing PC persistent directory can accept a new state file."""
  try:
...
def state_path() -> str:
  """The state file for tailscaled: data-root on device, persistent on PC when available."""
  if not PC:
...
def use_legacy_state() -> None:
  """Keep the PC daemon on its old state file after a failed persistent migration."""
  global _state_fallback
  _state_fallback = (persistent_state_path(), legacy_state_path())


def use_persistent_state() -> None:
  """Clear a previous PC migration fallback after persistence is available again."""
  global _state_fallback
...
def daemon_args() -> list[str]:
  ...
  # read-only rootfs on device. The state store is under data-root on device, or uses the
  # reset-surviving `--state` path on PC; binaries, the socket and this var root remain under
  # data-root `root()`.
