...
def _slam_node(sm, priors) -> Node | None:
  ...
  return Node(
    ...
    yaw_rate=-yaw_rate,
    trans_x=float(odometry.trans[0]),
    ...
    trans_y=-float(odometry.trans[1]),
    rot_z=-float(odometry.rot[2]),
    trans_std_x=float(odometry.transStd[0]),
    rot_std_z=float(odometry.rotStd[2]),
  )


def main() -> None:
  sm = messaging.SubMaster(['modelV2', 'radarState', 'carState', 'cameraOdometry', 'deviceMotion'], poll='modelV2')
