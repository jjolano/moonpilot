...
- **The lead's acceleration is the fork's own estimate, not radard's.** `aLeadK` is a `KF1D` with hardcoded gains at `DT_MDL` (closed-loop |λ| = 0.903, τ ≈ 0.49 s): 50 % of a −3 m/s² step at 0.60 s, 90 % at 1.20 s. `moonpilot/lead.py`'s `LeadAccelEstimator` takes the least-squares slope of `vLead` over a 0.35 s window instead — a clean ramp reads exactly once three samples are on it (0.10 s), a step onto a window that is already full in that window's own length (0.30 s), and it averages noise rather than lagging it: 0.19 m/s² of jitter over the window at 0.05 m/s of measurement noise — and falls back to `aLeadK` for a vision-only lead (where it is the model's own unfiltered accel) and whenever the window cannot speak: fewer than three samples, a new `radarTrackId`, a source flip, or a `vLead` jump past 2.5 m/s in a frame. Against a lead braking at −3.5 m/s² from a settled follow — clock: 0 on the frame whose sample is the first decelerated one — the command reaches −2.0 m/s² at 0.35 s, against 0.60 s for radard's own `aLeadK` through the same planner, and 1.45 s for the same planner reading the `aLeadK` field directly (the pinned test's shape, where every lead carries `a_lead=0.0` and radard's value is therefore absent entirely). The lead's *speed* stays radarState's raw `vLead`: it has no filter lag to remove — both arms measure the speed channel identically — and the same fit's endpoint value adds a transient bias exactly at the onset of braking. `aLeadTau` is radard's rule — 1.5 s until |a| passes 0.5 m/s², then a filter toward 0, re-armed to 1.5 whenever \|a\| falls back under the threshold or the `radarTrackId` changes — but driven by the fork's own estimate rather than by radard's `aLeadK`. That matters because the two disagree exactly at a brake onset: radard's Kalman is still under the reset threshold while the fork's estimate is already deep, so the message pairs a sustained-looking accel with a 1.5 s decay and the *published plan's* 2.5 s tail decays the braking away — the tail contradicts the command for 0.25–0.40 s after the onset, worst +0.24 m/s² against a −1.8 m/s² command. Driving the rule off the fork's estimate halves that window to 0.20–0.25 s. The command itself never sees any of it: the action horizon is 0.20 s, where the decay is 3 %. The residue is the decay *model*, not the pairing — `exp(-aLeadTau t²/2)` is radard's assumption that a lead's accel is a transient, and at 2.5 s that assumption is what is wrong; forcing `aLeadTau` to 0 puts the tail at −2.6 m/s² where the shipped estimate gives −0.15. Severity context: `longitudinalPlan.accels`/`speeds` has no consumer in openpilot outside `test/process_replay/migration.py`, so this whole tail question is log fidelity, not vehicle behavior.
