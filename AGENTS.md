...
`moonpilot/longitudinal.py` is the fork's own planner — what acceleration to ask for — and `moonpilot/longcontrol.py` its own acceleration controller, how that acceleration is tracked. Two seams pick them: `moonpilot_longitudinal_planner()` in plannerd, `moonpilot_longcontrol()` in controlsd, both factory calls returning `None` when the toggle is off, so upstream's acados MPC and upstream's PI loop stay on the line as the fallback. Seven things about them are not obvious:

- **The policy is closed-form, and it is three candidates.** A spacing regulator holding `gap == STOP_DISTANCE + t_follow * v_ego` with its braking authority capped at the approach decel; a time-to-collision approach term that holds the closing rate inside what the remaining slack affords at `TTC_TARGET` seconds — `closing <= slack / TTC_TARGET`, binding once that needs more than the approach decel, i.e. `slack < TTC_TARGET * (closing - APPROACH_DECEL / K_TTC)`: `slack < 5 * (closing - 1)` with `TTC_TARGET = 5.0`, `K_TTC = 1.0`; and the cruise term, plus the model's own accel in experimental mode. The smallest wins, ties keeping the earlier candidate. The plan's trajectory is the same policy rolled forward over the published horizon.
- **The handover into the TTC term is a step, and what bounds it is arbitration, not the handover.** Where the TTC term reaches the approach decel the regulator's output is whatever the spacing error says — positive while the gap is still wide, and at 25 m/s against a stopped lead that is +10.1 m/s² at the 126 m crossing (the old kinematic term crossed at 318 m, where the regulator still wanted +19.9 m/s²). It never reaches the output: while a lead candidate asks for more than the cruise candidate, `min` keeps cruise, so the step the output takes is cruise's cap minus the approach decel — 1.8 m/s² at 25 m/s, which the jerk limit turns into a rate. Upstream's maneuver suite has no maneuver in that regime, so `moonpilot/tests/test_longitudinal.py` pins it directly; what the TTC term buys is a response that keys off the lead's closing rate rather than its distance, at the price of a proportional term that saturates at `ACCEL_MIN` on a parked lead — 3.5 m/s² peak on the closed-loop stopped-lead approach, where the kinematic term held −1.0 m/s² for the whole of it, and the approach now begins at 147 m instead of 319 m.
...
- **The lead's acceleration is the fork's own estimate, not radard's.** `aLeadK` is a `KF1D` with hardcoded gains at `DT_MDL` (closed-loop |λ| = 0.903, τ ≈ 0.49 s): 50 % of a −3 m/s² step at 0.60 s, 90 % at 1.20 s. `moonpilot/lead.py`'s `LeadAccelEstimator` takes the least-squares slope of `vLead` over a 0.35 s window instead — a clean ramp reads exactly once three samples are on it (0.10 s), a step onto a window that is already full in that window's own length (0.30 s), and it averages noise rather than lagging it: 0.19 m/s² of jitter over the window at 0.05 m/s of measurement noise — and falls back to `aLeadK` for a vision-only lead (where it is the model's own unfiltered accel) and whenever the window cannot speak: fewer than three samples, a new `radarTrackId`, a source flip, or a `vLead` jump past 2.5 m/s in a frame. Against a lead braking at −3.5 m/s² from a settled follow — clock: 0 on the frame whose sample is the first decelerated one — the command reaches −1.0 m/s² at 0.45 s and −2.0 m/s² at 1.40 s, against 0.60 s and 2.50 s for the same planner reading radard's `aLeadK`/`aLeadTau` off the message (the pinned test's shape, where every lead carries `a_lead=0.0` and radard's value is therefore absent entirely). Both arms are later than they were under the kinematic term (estimator 0.25 s / 0.40 s, revert 0.55 s / 1.50 s), and for a reason worth knowing: the TTC approach term keys off the *closing rate*, which only grows once the lead's speed has fallen, where the kinematic term keyed off the lead's preview-corrected speed and so moved on the estimate directly. The cost is real and this is where it is paid: −1.0 m/s² arrives 0.20 s later than it did (0.45 s, 4 frames) and −2.0 m/s² a full second later (1.40 s, 20 frames). What survives is the *gap between the arms*, which is what this mechanism is for: the estimator still beats radard's pair by 0.15 s at −1.0 m/s² and by 1.10 s at −2.0 m/s² — the same 1.10 s it beat it by under the kinematic term. The lead's *speed* stays radarState's raw `vLead`: it has no filter lag to remove — both arms measure the speed channel identically — and the same fit's endpoint value adds a transient bias exactly at the onset of braking. `aLeadTau` is radard's rule — 1.5 s until |a| passes 0.5 m/s², then a filter toward 0, re-armed to 1.5 whenever \|a\| falls back under the threshold or the `radarTrackId` changes — but driven by the fork's own estimate rather than by radard's `aLeadK`. That matters because the two disagree exactly at a brake onset: radard's Kalman is still under the reset threshold while the fork's estimate is already deep, so the message pairs a sustained-looking accel with a 1.5 s decay and the *published plan's* 2.5 s tail decays the braking away — under the kinematic term the tail contradicted the command for 0.25–0.40 s after the onset, worst +0.24 m/s² against a −1.8 m/s² command, which driving the rule off the fork's estimate halved to 0.20–0.25 s. The TTC approach term has since moved where that question is observable, and the sample point moved with it — at 0.30 s past the onset the horizon's end now sits on the spacing regulator's −1.0 m/s² floor whichever decay is used (0.002 of separation), because the term drives it there; at 0.80 s the horizon reaches into the TTC term's active regime, where the fork's own decay (0.278 in the pinned scenario) puts the plan at −2.31 m/s² and radard's 1.5 s decay of the *same* accel floors it at −1.00 — 1.309 of separation. So `moonpilot/tests/test_longitudinal.py` pins it at 0.80 s, both arms on the estimator's accel and differing only in `aLeadTau`, which is the same contract the old 0.30 s sample carried and no longer can. What the pairing was fixing is unchanged underneath: `exp(-aLeadTau t²/2)` is radard's assumption that a lead's accel is a transient, and at 2.5 s that assumption is what is wrong — at the pinned sample's deepest point the same accel reads −2.31 m/s² on the fork's own 0.278 s decay, −3.08 m/s² with `aLeadTau` forced to 0 (no decay at all), and −1.00 m/s² on the message's 1.5 s. Severity context: `longitudinalPlan.accels`/`speeds` has no consumer in openpilot outside `test/process_replay/migration.py`, so this whole tail question is log fidelity, not vehicle behavior.
