...
- **The lead's acceleration is the fork's own estimate, not radard's.** `aLeadK` is a `KF1D` with hardcoded gains at `DT_MDL` (closed-loop |λ| = 0.903, τ ≈ 0.49 s): 50 % of a −3 m/s² step at 0.60 s, 90 % at 1.20 s. `moonpilot/lead.py`'s `LeadAccelEstimator` takes the least-squares slope of `vLead` over a 0.35 s window instead — a clean ramp reads exactly once three samples are on it (0.10 s), a step onto a window that is already full in that window's own length (0.30 s), and it averages noise rather than lagging it: 0.19 m/s² of jitter over the window at 0.05 m/s of measurement noise — and falls back to `aLeadK` for a vision-only lead (where it is the model's own unfiltered accel) and whenever the window cannot speak: fewer than three samples, a new `radarTrackId`, a source flip, or a `vLead` jump past 2.5 m/s in a frame. Against a lead braking at −3.5 m/s² from a settled follow, the command reaches −2.0 m/s² at 0.40 s, against 0.70 s with radard's own `aLeadK` and 1.50 s for the same planner reading the field directly (the pinned test's shape). The lead's *speed* stays radarState's raw `vLead`: it has no filter lag to remove, and the same fit's endpoint value adds a transient bias exactly at the onset of braking. Only `aLeadTau` is still radard's, and it only shapes the rollout's decay of the estimate: 3 % of it gone at the action horizon of the pinned test car (`action_t` = 0.20 s, 20 % on a Hyundai's 0.55 s), 99 % by the plan's 2.5 s tail. It is radard's *filtered* judgment, so a fork estimate deeper than radard's own `aLeadK` keeps the long decay until radard's Kalman catches up.
