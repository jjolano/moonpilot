...
| `openpilot/selfdrive/controls/controlsd.py` | `moonpilot_latcontrol()` picks the torque lateral controller, `moonpilot_longcontrol()` the acceleration controller, `moonpilot_curvature()` the path-preview curvature |
...
- **Latching is what makes an authoritative disable last, and it is released on an edge.** With `pcmDisable` and `pedalPressed` suppressed, the events that would keep re-disengaging are gone, so an authoritative disable (`USER_DISABLE`, `IMMEDIATE_DISABLE`) latches the state off in the module and only a re-arm gesture clears it: the cruise main switch coming back on, the LKAS button, or ACC's **rising** edge — not ACC merely being set. Clearing on the level would undo a driver's LKAS press one frame later for anyone cruising with ACC set, which makes their own off switch look broken; `moonpilot/tests/test_engage.py` pins both directions.
...
- **Two of upstream's disengages are inert here rather than suppressed**, and both are worth knowing before "fixing" the absence. Toyota's cancel button never reaches openpilot as `buttonCancel` — the car's wheel wires only `lkas` and `gapAdjustCruise` (`opendbc/car/toyota/carstate.py`), and `allow_button_cancel` only exempts Hyundai — so the driver's ACC cancel drops to `pcmDisable`, which is suppressed, and the car ends up half-engaged with steering intact; that is also the documented off gesture. And `steerDisengage` rides the same Tesla-only `steering_disengage` signal the safety rule's override term does.
...
The ceiling is the car: **Toyota/Lexus with stock longitudinal (`CP.pcmCruise`) that carries `PCM_CRUISE_2`**, not `passive` (a dashcam-mode car has already had its `safetyConfigs` replaced with a single noOutput config by the time the seam runs, so `_available()` checks the flag itself rather than trusting where the call sits), and not `ToyotaFlags.UNSUPPORTED_DSU` — those read the main switch out of `DSU_CRUISE` at 5 Hz, below panda's 10 Hz rx-check minimum, so `_available()` reports the feature unavailable rather than arming a check that would fail on the road. `PCM_CRUISE_2` is an rx check the fork added (33 Hz, checksum and counter ignored on purpose — a mismatch there would clear `controls_allowed` for *every* message, a worse failure than trusting a switch bit), and its declared rate is only a lag threshold: `safety_tick` trips at `max(10 · timestep, 1 s)`, and `is_msg_valid` can only fail on absence because all three flag checks are ignored. Since the message is on the bus of every in-scope platform — `MAIN_ON` bit 15 of `0x1D3`, from the same `_toyota_2017.dbc` signal `carstate.py` already reads for the cars without DSU — there is no plausible platform where the real rate is the problem. The symptom to look for is therefore not `safetyRxChecksInvalid`: with the feature on, a car that engages and does not steer, and then raises `controlsMismatch` about 2 s later, is the failure mode, and the first thing to check is whether `0x1D3` is being received at all rather than the declared number. `controlsd` is deliberately untouched: `CC.latActive` already follows `selfdriveState.active`, and `CC.longActive` already requires `CP.openpilotLongitudinalControl`, so the half-engaged state needs no second code path in the acceleration controller.
