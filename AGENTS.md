...
| `openpilot/selfdrive/controls/controlsd.py` | `moonpilot_latcontrol()` picks the torque lateral controller, `moonpilot_longcontrol()` the acceleration controller, `moonpilot_curvature()` applies response-aligned curvature, `moonpilot_path_smooth()` the optional curvature request filter, `moonpilot_actuator_gate()` both actuator permissions; `pandaStates` in the subscription |
...
| `openpilot/selfdrive/selfdrived/selfdrived.py` | `moonpilot_engage()`; `LateralEngage.update()` after every event source; the panda cross-check; `is_fork_build()` picks the startup banner; the turn-desire banner, raised from `moonpilot/turn_desire.py`'s `turn_desire_alert` — the fed desire is never published, so the banner recomputes the same predicate modeld feeds the model, and requires `carControl.latActive` so it appears only while openpilot is the thing steering; the path-outside banner from `moonpilot/corridor.py`'s `path_outside_alert` |
| `openpilot/selfdrive/selfdrived/events.py` | `EventName.lateralEngageOff` — the half-engagement banner; the turn-desire banner; the path-outside banner |
...
| `openpilot/selfdrive/ui/onroad/augmented_road_view.py` | half-engaged border color from `moonpilot/ui/onroad.py`; the disengaged border color moves off blue; the offroad-mode hold gesture; the wide-camera-at-low-speed gate, which also opens for the fork planner |
| `openpilot/selfdrive/ui/onroad/model_renderer.py` | lead path draw (tizi) |
| `openpilot/selfdrive/ui/mici/onroad/augmented_road_view.py` | the offroad-mode hold gesture (mici); the wide-camera-at-low-speed gate (mici) |
...
`moonpilot/curvature.py` aligns the model's path with when steering is expected to respond, as a bounded correction toward the path anchored on `modelV2.action.desiredCurvature`; the seam is `moonpilot_curvature()` in controlsd, chosen once at construction, so the offroad-only row takes a restart. Stale, future-dated, malformed or out-of-grid input returns `desiredCurvature` bit-for-bit. The response horizon runs on upstream `lagd`'s `lateralDelay`, which sub-50-mph driving cannot validate — a coverage statement, not a defect. The same file holds `moonpilot_path_smooth()`, the optional request filter (`MoonpilotPathSmooth`, off by default): a control-rate IIR on `new_desired_curvature` before `clip_curvature`, also chosen at construction, which tracks the actual curvature while not steering so a re-engage starts from the car's path, and whose τ folds into **both** `lat_delay` reads — the reference's sampling horizon and the controller's — so the lag it adds is never invisible to either consumer. `moonpilot/tests/test_curvature.py` pins it. Full details: `moonpilot/docs/curvature.md`.

### The longitudinal strategy

`moonpilot/longitudinal.py` is the fork's own planner and `moonpilot/longcontrol.py` its own acceleration controller; the seams are `moonpilot_longitudinal_planner()` in plannerd and `moonpilot_longcontrol()` in controlsd, both factory calls returning `None` when the toggle is off, so upstream's MPC and PI loop stay on the line. The policy is three closed-form candidates (`min` wins): a spacing regulator with a standstill floor under the headway, a TTC approach term, a stopping floor in the relative frame with a bounded lead-brake sustain — plus cruise, grade coasting, curve speed, corridor squeeze braking from `modelV2.roadEdges` (pose-stitched over a short `egoPose` history when a valid pose is published, single-frame otherwise), and the model's braking ask through a deadband that, once past it, holds a stop the model began until the model asks to go. The regulator's **positive** side runs on less authority above 20 m/s (`MOONPILOT_POS_AUTH_V`, 1.0 below it), because a follow ask that reaches the cruise candidate's saturated fast-accel rung stops being proportional: "slightly behind" and "far behind" become the same command. It is a multiplier on the regulator's *output* and only where that is positive — on the two terms instead, the opposite-signed states move toward zero and a brake gets weaker — so the braking side is bit-identical. The regulator's *negative* side is separately not the gap target's to enforce while the ego is the slower car (`MOONPILOT_FOLLOW_SPEED_FLOOR_HEADWAY_T`): at a set speed above the lead's the 1.45 s headway is unreachable at whatever speed the lead chooses, so the gap term alone demanded the full approach brake at *exactly* the lead's speed and the ego decayed to below it and stayed there. `MoonpilotLeadLateral`'s out-of-path headway reduction is a latched two-state claim on `inPath` (`MOONPILOT_OUT_OF_PATH_ENTER`/`_LEAVE`), not a continuous scale, because `inPath` is a weak signal at speed and a linear map put a moving target under that regulator. Both seams are chosen once at construction, so the row takes a restart and only applies to `openpilotLongitudinalControl` cars. `moonpilot_longitudinal_active()` is that same decision without building a planner, and the UI reads it: both road views widen to the wide road camera below their speed threshold when the fork planner is the one in place, not only in experimental mode. That is the fork's counterpart of upstream's gate, and the two are disjoint — an `openpilotLongitudinalControl` car is exactly the one where experimental mode is unavailable. `moonpilot/tests/test_longitudinal.py`, `test_longitudinal_follow.py` (the follow policy's own file, split out because `test_longitudinal.py` sits at the 120 KB lint gate), `test_longitudinal_features.py` (curve/squeeze through the planner seam) and `test_longcontrol.py` pin it — pure corridor math in `test_corridor.py` — and `test_longitudinal.py` ends by running upstream's maneuver suite against the fork planner. Full details of every non-obvious property: `moonpilot/docs/longitudinal.md`.

### The rolling-window ego correction

`moonpilot/slam.py` blends the wheel-speed prior against camera odometry's forward translation over a 5 s window and publishes the disagreement as `moonpilotState.egoCorrection`, which the longitudinal planner adds to `v_ego` — one added number, so the pass-through case (off, invalid, stale, dead publisher) is the stock planner exactly. One process publishes `moonpilotState` and it is `leadd`; a second publisher would take the leads down with it. The correction is `smoothed minus the raw prior` (sign load-bearing), the gate is live not restart-gated, and a standstill gets nothing. The same publisher also fills `moonpilotState.egoPose`, whose first control-path consumer is the pose-stitched corridor history above: without a valid pose that consumer is the single-frame squeeze it had before. `moonpilot/tests/test_slam.py` pins the sign, clamps, staleness and calibration ingest. Full details: `moonpilot/docs/slam.md`.
...
A driver may replace the model this car drives: panes in `moonpilot/ui/models.py`/`models_mici.py`, the tizi picker in `moonpilot/ui/model_picker.py`, rules in `moonpilot/models.py`, catalog in `moonpilot/modelcatalog.py`, worker in `moonpilot/modelsd.py`, compiler in `moonpilot/modelbuild.py`, runtime in `moonpilot/modelruntime.py`. A selection is a string (`""` bundled, else a 64-hex digest), committed once at boot by `commit_boot_selection` into `MoonpilotModelsBoot`, which the model processes read and nothing else — the row takes a restart. A load failure falls back to the bundled model and takes the whole runtime with it, action decode included. Only four protocols are selectable; the worker never exits on its own and never compiles onroad; nothing on the boot path touches the network. Three ceilings: 300 MB per artifact, 8 GB store, 2 GB free space left alone. `moonpilot/tests/test_models.py`, `test_modelcatalog.py` and `test_modelruntime.py` check it without a car. Full details: `moonpilot/docs/models.md`.

The picker is a tree, shaped like sunnypilot's: the model in effect pinned at the top, `+`/`-` group headers, a star per row into `MoonpilotModelsFavs` (`;`-joined digests, stale ones free), a search box, and a footer sentence saying what choosing the highlighted row does — tizi gets a dialog, mici a two-level drill-down because a card column has no room for a tree. **Groups and names are the catalog's**: `browse_index` copies openmodels' own `folder`/`short_name`/`name_kind` claims and the picker prints them verbatim, and a row without them falls back to the group the fork owns — the protocol the recipe needs, then the model class. Do not add a fork-side name or folder table: it would rot against every catalog refresh, and openmodels' reviewed naming records are where a name is earned. The one line in `moonpilot/vendor/openmodels/contracts.py` that admits those two keys is load-bearing rather than a fork extension — the catalog's `model` object is closed, so without it a refreshed catalog is rejected whole.
